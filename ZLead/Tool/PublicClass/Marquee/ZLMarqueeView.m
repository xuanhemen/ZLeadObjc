...
- (instancetype)initWithFrame:(CGRect)frame withTitle:(NSArray *)titles {
    if (self = [super initWithFrame:frame]) {
        ...
        self.index = 1;
        self.firstBtn = [self btnframe:CGRectMake(CGRectGetMaxX(speakerIcon.frame), 0, [UIScreen mainScreen].bounds.size.width - 2 *30, self.bounds.size.height)  titleColor:_titleColor action:@selector(clickBtn:)];

...
- (void)setupView{
    ...
    if (_sheetTitle.length != 0) {
        ...
        _titleLine = [[UIView alloc] initWithFrame:CGRectMake(0, _titleHeight-0.5, ScreenWidth, 0.5)];
        _titleLine.backgroundColor = [UIColor grayColor];
        [_headerView addSubview:_titleLine];
...
- (void)setSheetProperty{
    ...
    if (self.cancleFont) {
        _cancleBtn.titleLabel.font = self.cancleFont;
    }
    if (self.titleLineColor) {
        _titleLine.backgroundColor = self.titleLineColor;
    }
    if (self.isCorner) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.hideLastLine && indexPath.row==_cellCount-1) {
